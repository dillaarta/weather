...
### PURPOSE
Create table for dataset_review from yelp_academic_dataset_review
...
def get_data_load():
    ...
    blobs = storage_client.list_blobs(bucket_name,prefix='weather/yelp_academic_dataset_review/')
    all_name = []
    for blob in blobs:
    ...
    for files in new:
        ...
        for i in json.loads(blob.download_as_string()):
            df = pd.json_normalize(i,max_level=4)
            df = df.applymap(str)
...
with DAG(
        dag_id='yelp_academic_dataset_review',
    ...
    with TaskGroup("dataset_review") as extract_group:
        create_table = PostgresOperator(      
                task_id = "create_table_task",
                postgres_conn_id = "POSTGRES",
                sql = 'utils/sql/create_table_review.sql',
                dag = dag
                )
